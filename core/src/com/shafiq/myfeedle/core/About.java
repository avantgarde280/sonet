...
public class About extends ListActivity implements DialogInterface.OnClickListener {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (itemId == R.id.menu_about_refresh) {
			startService(Myfeedle.getPackageIntent(this, MyfeedleService.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, AppWidgetManager.INVALID_APPWIDGET_ID).setAction(ACTION_REFRESH));
		} else if (itemId == R.id.menu_about_accounts_and_settings) {
			startActivity(Myfeedle.getPackageIntent(this, ManageAccounts.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, AppWidgetManager.INVALID_APPWIDGET_ID));
		} else if (itemId == R.id.menu_about_default_settings) {
			startActivityForResult(Myfeedle.getPackageIntent(this, Settings.class), RESULT_REFRESH);
		} else if (itemId == R.id.menu_about_refresh_widgets) {
			startService(Myfeedle.getPackageIntent(this, MyfeedleService.class).setAction(ACTION_REFRESH).putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS, mAppWidgetIds));
	...
	protected void onResume() {
		...
		mAppWidgetIds = Myfeedle.getWidgets(getApplicationContext(), mAppWidgetManager);
	...
	protected void onPause() {
		...
		if (mUpdateWidget) {
			...
			startService(Myfeedle.getPackageIntent(this, MyfeedleService.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS, mAppWidgetIds));
		}
	}
	...
	public void onClick(DialogInterface dialog, int which) {
		startActivity(Myfeedle.getPackageIntent(this, ManageAccounts.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetIds[which]));
		dialog.cancel();
	}
	...
	protected void onListItemClick(ListView list, final View view, int position, long id) {
		...
		Rect r = new Rect();
		view.getHitRect(r);
		startActivity(Myfeedle.getPackageIntent(this, StatusDialog.class).setData(Uri.withAppendedPath(Statuses_styles.getContentUri(this), Long.toString(id))).putExtra(LauncherIntent.Extra.Scroll.EXTRA_SOURCE_BOUNDS, r).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
	}
...
//			Cursor widgets = getContentResolver().query(Widgets.getContentUri(About.this), new String[] {Widgets._ID, Widgets.WIDGET}, Widgets.ACCOUNT + "=?", new String[] { Long.toString(Myfeedle.INVALID_ACCOUNT_ID) }, null);
...
//					if ((appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) && !Myfeedle.arrayContains(mAppWidgetIds, appWidgetId)) removeAppWidgets = Myfeedle.arrayAdd(removeAppWidgets, appWidgetId);
//					widgets.moveToNext();
//				}
//			}
//			widgets.close();
...
//			}
			...
			if (accounts.moveToFirst()) {
				...
				if (widget.moveToFirst()) {
					...
					values.put(Widgets.ACCOUNT, Myfeedle.INVALID_ACCOUNT_ID);
		...
		protected void onPostExecute(Integer result) {
			...
			if (result > 0) {
				...
				startService(Myfeedle.getPackageIntent(About.this, MyfeedleService.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, AppWidgetManager.INVALID_APPWIDGET_ID).setAction(ACTION_REFRESH));
				if (result > 1) {
					// no accounts
