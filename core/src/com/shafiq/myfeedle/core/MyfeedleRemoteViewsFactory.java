...
import static com.shafiq.myfeedle.core.Myfeedle.getBlob;
import static com.shafiq.myfeedle.core.Myfeedle.sBFOptions;

@SuppressLint("NewApi")
public class MyfeedleRemoteViewsFactory implements android.widget.RemoteViewsService.RemoteViewsFactory {
	private static final String TAG = "MyfeedleRemoteViewsFactory";
	private Context mContext;
	private Cursor mCursor;
	private int mAppWidgetId;
	private boolean mDisplay_profile;

	public MyfeedleRemoteViewsFactory(Context context, Intent intent) {
		mContext = context;
	...
	public RemoteViews getViewAt(int position) {
		...
		if ((mCursor != null) && !mCursor.isClosed() && mCursor.moveToPosition(position)) {
			...
			extras.putString(Myfeedle.Status_links.STATUS_ID, Long.toString(mCursor.getLong(0)));
