...
import static com.shafiq.myfeedle.core.Myfeedle.map_icons;
import static com.shafiq.myfeedle.core.Myfeedle.sBFOptions;
import static com.shafiq.myfeedle.core.MyfeedleProvider.TABLE_ACCOUNTS;
import static com.shafiq.myfeedle.core.MyfeedleProvider.TABLE_WIDGETS;
import static com.shafiq.myfeedle.core.MyfeedleProvider.TABLE_WIDGET_ACCOUNTS;
...
public class ManageAccounts extends ActionBarListActivity implements DialogInterface.OnClickListener {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (itemId == R.id.button_add_account) {
			...
			mAddingAccount = true;
			startActivityForResult(Myfeedle.getPackageIntent(this, Settings.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId), RESULT_REFRESH);
		}
	...
	protected void onListItemClick(ListView list, final View view, int position, final long id) {
		...
		.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				case REAUTH_ID:
					...
					if (c.moveToFirst()) {
						...
						if ((service != SMS) && (service != RSS)) {
							mAddingAccount = true;
							startActivityForResult(Myfeedle.getPackageIntent(ManageAccounts.this, OAuthLogin.class).putExtra(Accounts.SERVICE, service).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId).putExtra(Myfeedle.EXTRA_ACCOUNT_ID, id), RESULT_REFRESH);
						}
					}
					c.close();
					break;
				case SETTINGS_ID:
					mAddingAccount = true;
					startActivityForResult(Myfeedle.getPackageIntent(ManageAccounts.this, AccountSettings.class).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId).putExtra(Myfeedle.EXTRA_ACCOUNT_ID, id), RESULT_REFRESH);
					break;
				case ENABLE_ID:
	...
	private void listAccounts() {
		...
		Cursor c = this.managedQuery(Accounts_styles.getContentUri(this), new String[]{
				...
				"(case when " + Accounts.SERVICE + "=" + Myfeedle.TWITTER + " then 'Twitter: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.FACEBOOK + " then 'Facebook: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.MYSPACE + " then 'MySpace: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.LINKEDIN + " then 'LinkedIn: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.FOURSQUARE + " then 'Foursquare: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.CHATTER + " then 'Chatter: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.RSS + " then 'RSS: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.IDENTICA + " then 'Identi.ca: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.GOOGLEPLUS + " then 'Google+: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.PINTEREST + " then 'Pinterest: ' else '' end) as " + Statuses_styles.FRIEND,
				...
				+ Accounts.SERVICE + "=" + Myfeedle.IDENTICA + " then 'Identi.ca: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.GOOGLEPLUS + " then 'Google+: ' when "
				+ Accounts.SERVICE + "=" + Myfeedle.PINTEREST + " then 'Pinterest: ' else '' end) as " + Statuses_styles.FRIEND + "2",
				...
				+ "else " + Myfeedle.default_message_bg_color + " end) as " + Statuses_styles.STATUS_BG,
				...
				+ "else " + Myfeedle.default_message_bg_color + " end) as " + Statuses_styles.PROFILE_BG,
				...
				+ "else " + Myfeedle.default_friend_bg_color + " end) as " + Statuses_styles.FRIEND_BG
		}, null, null, null);
	...
	public void onClick(DialogInterface dialog, int which) {
		mAddingAccount = true;
		startActivityForResult(Myfeedle.getPackageIntent(this, OAuthLogin.class).putExtra(Accounts.SERVICE, Integer.parseInt(getResources().getStringArray(R.array.service_values)[which])).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, mAppWidgetId).putExtra(Myfeedle.EXTRA_ACCOUNT_ID, Myfeedle.INVALID_ACCOUNT_ID), RESULT_REFRESH);
		dialog.cancel();
	}
